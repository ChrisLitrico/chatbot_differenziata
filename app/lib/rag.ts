...
const INDEX_NAME = process.env.MONGODB_VECTOR_INDEX || 'default'
const VECTOR_PATHS = process.env.MONGODB_VECTOR_PATH ? [process.env.MONGODB_VECTOR_PATH] : ['embedding', 'docs.embedding']
...
async function vectorSearch(query: string, options?: { limit?: number; filter?: Record<string, unknown> }): Promise<RetrievedDoc[]> {
  ...
  const limit = options?.limit ?? FETCH_RESULTS
  let queryEmbedding: number[] | null = null

  // Prova a generare l'embedding solo se necessario
  try {
    ...
    // Se l'embedding fallisce, vai direttamente al fallback testuale
    return await textFallbackSearch(normalizedQuery, options?.filter)
  }

  for (const vectorPath of VECTOR_PATHS) {
    try {
      ...
      if (options?.filter) {
        vectorStage.filter = options.filter
      }
      ...
      console.error(`[RAG] vectorSearch failed for path="${vectorPath}":`, error)
    }
  }

  // Se vector search ha fallito o non ha trovato risultati, usa fallback testuale
  return await textFallbackSearch(normalizedQuery, options?.filter)
}

// Fallback: ricerca testuale quando vector search non restituisce risultati
async function textFallbackSearch(query: string, filter?: Record<string, unknown>): Promise<RetrievedDoc[]> {
  const collection = await getCollection()
  ...
  // Usa una singola regex combinata invece di multiple $or (più veloce)
  const combinedPattern = keywords.join('|')
  const textQuery: Record<string, unknown> = {
    text: { $regex: combinedPattern, $options: 'i' },
  }

  // Aggiungi filtro per comune se presente
  if (filter && '$or' in filter) {
    Object.assign(textQuery, { $and: [{ text: textQuery.text }, filter] })
    delete textQuery.text
  } else if (filter) {
    Object.assign(textQuery, filter)
  }
...
function comuneFilter(comune: string) {
  return {
    $or: [{ comune }, { 'metadata.comune': comune }],
  }
}
...
export async function retrieveRelevantDocuments(query: string): Promise<RetrievedDoc[]> {
  ...
  try {
    ...
    // Se c'è un comune, cerca direttamente con filtro (più veloce)
    const searchOptions = targetComune ? { filter: comuneFilter(targetComune) } : undefined
    const baseDocs = await vectorSearch(normalizedQuery, searchOptions)
    ...
    // Prioritizza per comune se rilevato
    if (targetComune) {
